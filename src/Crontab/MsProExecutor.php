...
use MsPro\Crontab\Mutex\RedisServerMutex;
use MsPro\Crontab\Mutex\RedisTaskMutex;
use MsPro\Crontab\Mutex\ServerMutex;
use MsPro\Crontab\Mutex\TaskMutex;
...
class MsProExecutor
{
     ...
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function execute(MsProCrontab $crontab, bool $run = false): ?bool
    {
        if ((! $crontab instanceof MsProCrontab || ! $crontab->getExecuteTime()) && !$run) {
            return null;
        }
        $diff = 0;
    ...
    protected function runInSingleton(MsProCrontab $crontab, Closure $runnable): Closure
    {
    ...
    protected function runOnOneServer(MsProCrontab $crontab, Closure $runnable): Closure
    {
        return function () use ($crontab, $runnable) {
     ...
     * @return Closure
     */
    protected function decorateRunnable(MsProCrontab $crontab, Closure $runnable): Closure
    {
    ...
    protected function logResult(MsProCrontab $crontab, bool $isSuccess, $result = '')
    {
        if ($this->logger) {
            if ($isSuccess) {
        ...
        $data = [
            ...
            'status' => $isSuccess ? MsProModel::ENABLE : MsProModel::DISABLE,
