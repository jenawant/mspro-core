 ...
 */
#[Command]
class InstallProjectCommand extends MsProCommand
{
    /**
     * 安装命令
     * @var string|null
     */
    protected ?string $name = 'mspro:install';
    ...
    {
        parent::configure();
        $this->setHelp('run "php bin/hyperf.php mspro:install" install MsProAdmin system');
        $this->setDescription('MsProAdmin system install command');

        $this->addOption('option', '-o', InputOption::VALUE_OPTIONAL, 'input "-o reset" is re install MsProAdmin');
    }
    ...
    {
        $this->line('-----------------------------------------------------------', 'comment');
        $this->line('Hello, welcome use MsProAdmin system.', 'comment');
