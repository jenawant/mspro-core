...
trait MapperTrait
{
    ...
    {
        if (! config('msproadmin.remote_api_enabled')) {
            throw new MsProException('系统未启用【远程通用列表查询】', 500);
        }
        /* @var $model MsProModel */
        $model = $this->getModel();
        $query = null;
        ...
        return method_exists($this, 'handleItems')
            ? ( new MsProCollection($this->handleItems($query->get($params['select'] ?? ['*']))) )->toArray()
     ...
     * @return mixed
     */
    public function first(array $condition, array $column = ['*']): ?MsProModel
    {
     ...
     * @return MsProModel
     */
    public function getModel(): MsProModel
    {
    ...
    {
        return (new MsProCollection())->import($dto, $this->getModel(), $closure);
    }
     ...
     * @return void
     */
    public function dynamicRelations(\MsPro\MsProModel &$model, &$config): void
    {
