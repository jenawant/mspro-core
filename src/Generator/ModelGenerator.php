...
class ModelGenerator extends MineGenerator implements CodeGenerator
{
    ...
    public function generator(): void
    {
        ...
        if ($code === 0) {
            ...
            throw new NormalStatusException(t('setting.gen_model_error') . $code, 500);
        }
    }
     ...
     * @return string
     */
    protected function getUseProperties(): string
    {
        $properties = [];
        $softDelete = make(SettingGenerateColumnsService::class)->mapper->first(['table_id' => $this->model->id, 'column_name' => 'deleted_at']);
        if ($softDelete) {
        ...
        if (!$incrementing) {
            ...
            foreach ($columns as $column) {
                if (str_contains($column['column_key'], 'PRI')) {
                    $properties[] = "protected \$primaryKey = '" . $column['column_name'] . "';";
                    break;
                }
            }
        }
    ...
    protected function getProperties(): string
    {
        ...
        foreach ($data as $column) {
            ...
            $columns[] = " * @property {$column['column_type']} \${$column['column_name']} {$column['column_comment']}";
        }
